...
const calculateExtractedText = function(inText) {
  ...
  ps.forEach(function(paragraph) {
    ...
    const ts = xpath.select("//*[local-name()='t' or local-name()='tab' or local-name()='br' or local-name()='instrText']", paragraph);
        ...
        const processEnd = function() {
          if (zipfile.entryCount === ++processedEntries) {
            if (result.length) {
              ...
              return reject(new Error("Extraction could not find content in file"));
            }
          }
        };

        zipfile.on('entry', function(entry) {
          if (/\.xml$/.test(entry.fileName) && (!/^(word\/media\/|word\/_rels\/)/.test(entry.fileName))) {
            return getTextFromZipFile(zipfile, entry)
              .then(function(data) {
